...
int Queue_IsFull( const Queue *queue ) {
	// if we get the next index of free index in the queue and it is the same number
	// as first index it means the queue is full, since we got to the start again
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	...
	// check if the queue is empty and return error if it is
...
void Queue_Remove( Queue *queue ) {
	...
	// set the first index in the queue to the next one
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	...
	// check if the queue is empty and returns an error if it is
	if(Queue_IsEmpty(queue)) {
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	// check if the queue is full and return error if it is
	if(Queue_IsFull(queue)) {
		Queue_Error(QERR_ENQUEUE);
		return;
	}

	// put the data in the first free index
