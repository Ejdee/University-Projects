...
class IsBlockCommand implements IBuiltInMethodCommand
{
    ...
    {
        if (count($args) !== 0) {
            throw new OtherRuntimeException("Invalid arguments count for isBlock method.");
        }

        if ($this->instanceTable->isInstanceOf("Block", $receiver)) {
